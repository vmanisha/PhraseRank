...
public class phraseRankHolder {
	...
	public Vector <Float> findTop50Prec()
	{
		float top50=0;
		int phrase;
		int ctr=0;
		...
		float rel=0.0f,pre=0.0f;
		Vector <Float> precision= new Vector <Float>();
		for(int i=0;i<ctr;i++)
		{
			phrase=phraseNo.elementAt(i);
			if(origRank.indexOf(phrase)<=50 && origRank.indexOf(phrase)>0)
			{
				pre++;
				rel++;
			}
	...
	public Vector <Float> findTop50Recall()
	{
		float top50=0;
		int phrase;
		int ctr=0;
